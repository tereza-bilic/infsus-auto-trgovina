...
export const validateServisHistory = async (servisHistory: {
  ...
  if (previousHistory) {
    if (
      servisHistory.trenutnaKilometraza <= previousHistory.trenutnaKilometraza
    ) {
      throw new Error(
        "Trenutna kilometraža mora biti veća od kilometraže prethodnog servisa."
      );
    }
  }
};

export const getAllServisHistories = async (): Promise<
  PovijestServisiranja[]
> => {
  return (await prisma.povijestServisiranja.findMany()).sort(
    (a, b) => a.datum.getTime() - b.datum.getTime()
  );
};

export const getServisHistoryById = async (
  id: number
): Promise<PovijestServisiranja | null> => {
  return prisma.povijestServisiranja.findUnique({
    where: { id },
  });
};

export const getServiceHistoryByOglasId = async (
  oglasId: number
): Promise<PovijestServisiranja[]> => {
  return (
    await prisma.povijestServisiranja.findMany({
      where: { oglasId },
    })
  ).sort((a, b) => a.datum.getTime() - b.datum.getTime());
};

export const createServisHistory = async (
  data: Omit<PovijestServisiranja, "id">
): Promise<PovijestServisiranja> => {
...
export const updateServisHistory = async (
  id: number,
  servisHistory: {
    datum: Date;
    serviser: string;
...
export const deleteServisHistory = async (
  id: number
): Promise<PovijestServisiranja> => {
